...
var ObjectStore = Class({
  ...
  initialize: function initialize(store, db) {
    this.store = store;
    this.db = db;
  },
  ...
  createIndex : function createIndex(name, keyPath, options) {
    ...
    return this.db._createIndex(this.store, name, keyPath, options);
  },
});
  ...
  _createIndex : function _createIndex(store, name, keyPath, options) {
    ...
    request.addEventListener("success", function (event) {
      if (success) {
        ...
        emit(database, "objectstore:index", index);
        resolve(index);
      } else {
        reject(event);
      }
    ...
    request.addEventListener("upgradeneeded", function (event) {
      ...
      try {
        ...
        event.target.transaction.objectStore(store.name).createIndex(name, keyPath, options);
        // set our success flag to true
        success = true;
      } catch (e) {
        console.error(e);
